...
def computeMetrics(Joint, gtFramesAll, motAll, outputDir, bSaveAll, bSaveSeq):
    ...
    for si in range(nSeq):
        ...
        if (bSaveSeq):
            ...
            filename = os.path.join(outputDir, seqName+'_MOT_metrics.json')
...
def evaluateTracking(Joint, gtFramesAll, prFramesAll, outputDir, saveAll=True, saveSeq=False):

    distThresh = 0.5
    # assign predicted poses to GT poses
    _, _, _, motAll = eval_helpers.assignGTmulti(Joint, gtFramesAll, prFramesAll, distThresh)

    # compute MOT metrics per part
    metricsAll = computeMetrics(Joint, gtFramesAll, motAll, outputDir, saveAll, saveSeq)
